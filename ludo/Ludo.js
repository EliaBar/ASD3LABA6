...
export class Ludo {
    ...
    checkForEligiblePieces() {
        ...
        if(eligiblePieces.length) {
            ...
            if(this.turn === 1 ){
                let newPiece = this.getBestPieceForMove(player, eligiblePieces);
                this.handlePieceClick(player, newPiece);
            }
        } else {
            this.incrementTurn();
        }
    ...
    getBestPieceForMove(player, eligiblePieces) {
        let bestPiece = null;
        let bestScore = -Infinity;

        eligiblePieces.forEach(piece => {
            const score = this.evaluatePiece(player, piece);
            if (score > bestScore) {
                bestScore = score;
                bestPiece = piece;
            }
    ...
    evaluatePiece(player, piece) {
        ...
        if (SAFE_POSITIONS.includes(currentPosition + this.diceValue)) {
            let opponentFound = false;

            for (let i = 1; i <= 6; i++) {
                let checkPosition = currentPosition - i;
        
                if (this.isOpponentPieceAtPosition(checkPosition, piece)) {
                    opponentFound = true;
                    break;
                }
            }
        ...
        if (this.checkForKill(player, piece)) {
            score += 3;
        }
    ...
    isOpponentPieceAtPosition(position, piece) {
        if (this.turn === 0) { 
            return position == this.currentPositions['P1'][piece];
        } else { 
            return position == this.currentPositions['P2'][piece];
        }
    }
